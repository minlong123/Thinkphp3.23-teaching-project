		 ...
		 <div class="bs-example" data-example-id="table-within-panel">
		    <div class="panel panel-default">
		      ...
		      <div class="panel-body">
		      	<?php if(session('jur') == '0' or strstr(session('jur'),'Category-add')): ?><p>可增加修改删除导航</p>
		        <a href="/Admin/Category/add" class="btn btn-primary">添加导航</a><?php endif; ?>
					    ...
					    <div class="panel panel-default">
					      <div class="panel-heading" id="collapseListGroupHeading">
					        <h4 class="panel-title">
					          ...
					          <?php if(isset($vo['items'])): ?><a href="#collapseListGroup<?php echo ($vo['id']); ?>" data-toggle="collapse"  class="btn btn-primary" style="color:#fff;">展开</a>
					          <?php else: ?>
					          <button class="btn" style="visibility:hidden;">&nbsp;</button><?php endif; ?>

					          <?php if(session('jur') == '0' or strstr(session('jur'),'Category-edit')): ?><a href="/Admin/Category/edit/id/<?php echo ($vo['id']); ?>" class="btn btn-warning edit-nav">修改</a><?php endif; ?>

					          <?php if(session('jur') == '0' or strstr(session('jur'),'Category-deleone')): ?><a href="javascript:void(0)" class="btn btn-danger delte-nav" data-toggle="modal" onclick="nav.deleone(<?php echo ($vo['id']); ?>)">删除</a><?php endif; ?>	
					      ...
					      <?php if(isset($vo['items'])): ?><div id="collapseListGroup<?php echo ($vo['id']); ?>" class="panel-collapse collapse" role="tabpanel" aria-labelledby="collapseListGroupHeading1">
						        <ul class="list-group">
						          ...
						          <?php if(is_array($vo['items'])): foreach($vo['items'] as $key=>$voo): ?><li class="list-group-item">
						          	...
						          	<?php if(session('jur') == '0' or strstr(session('jur'),'Category-edit')): ?><a href="/Admin/Category/edit/id/<?php echo ($voo['id']); ?>" class="btn btn-warning edit-nav">修改</a><?php endif; ?>

						          	<?php if(session('jur') == '0' or strstr(session('jur'),'Category-dele')): ?><a href="javascript:void(0)" class="btn btn-danger delte-nav" data-toggle="modal" onclick="nav.delenav(<?php echo ($voo['id']); ?>)">删除</a><?php endif; ?>
	...
	<script type="text/javascript">
		...
		nav.delenav=function(id){
		  ...
		  $('.detet').click(function(){
		  	 $.post("/Admin/Category/dele",{id:id},function(res,tip){
		  	 	if(tip=="success"){
		  	 		window.history.go(0);
		  	 	}
		  	 })
		...
		nav.deleone=function(id){
		  ...
		  $('.detet').click(function(){
		  	...
		  	$('.detett').click(function(){
			  	$.post("/Admin/Category/deleone",{id:id},function(res,tip){
			  	 if(tip=="success"){
			  	 	window.history.go(0);
			  	 }
			  	})
