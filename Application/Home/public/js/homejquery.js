...
// 因为每次打开一个新的页面，导航的数据都是从后台请求的,所以之前加的class就不存在了，
// 所以这里不需要清除前面设置的class
			function navShow(){
				...
				$('.navulli').each(function(){
					(function(u,_t){
						_t.find("a").each(function(){
							if(u.indexOf($(this).attr("href"))!=-1){
								_t.addClass('select_black');
							}
						})
					})(url,$(this));

				});
				if(url=="http://localhost:82/" || url=="localhost:82"){
					$('.navulli:first-child').addClass('select_black');
				}
			}
			navShow();
